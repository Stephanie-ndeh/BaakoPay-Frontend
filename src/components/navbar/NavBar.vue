...
const links = [
  { name: "Home", path: { name: "landing" } },
  { name: "About Us", path: { name: "landing" } },
  { name: "Features", path: { name: "landing" } },
];
const isActiveLink = (linkPath: { name: string }) => {
  return computed(() => route.name === linkPath.name);
};
</script>
            ...
            <RouterLink
              ...
              :class="[
                isActiveLink(link.path).value
                  ? ' text-primary-100 lg:text-secondary-200 font-medium'
                  : 'lg:text-white ',
              ]"
